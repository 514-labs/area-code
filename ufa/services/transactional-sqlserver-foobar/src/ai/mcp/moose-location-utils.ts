      ...
      if (fs.existsSync(searchPath)) {
        ...
        for (const subdir of subdirs) {
          ...
          if (fs.existsSync(configPath)) {
            ...
            if (subdir.includes("analytical-sqlserver")) {
              return servicePath;
            }
          }
        }
...
export function findAnalyticalMooseServicePath(currentDir: string): string {
  ...
  const possiblePaths = [
    path.resolve(workspaceRoot, "ufa/services/analytical-sqlserver-moose-foobar"),
    // Add other possible locations if the service gets moved
    path.resolve(workspaceRoot, "services/analytical-sqlserver-moose-foobar"),
  ];
